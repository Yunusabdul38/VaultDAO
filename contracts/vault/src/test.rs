...
fn make_client(
    ...
    client.initialize(&admin, &config);
    client.set_role(&admin, &signer1, &Role::Treasurer);
    (client, admin, signer1, user, token)
}

#[test]
fn test_metadata_valid_value() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.set_proposal_metadata(&signer1, &proposal_id, &key, &value);
    let proposal = client.get_proposal(&proposal_id);
    assert_eq!(proposal.metadata.get(key), Some(value));
}
...
fn test_metadata_empty_value_rejected() {
    ...
    let res = client.try_set_proposal_metadata(&signer1, &proposal_id, &key, &empty);
    assert_eq!(res.err(), Some(Ok(VaultError::MetadataValueInvalid)));
}
...
fn test_metadata_value_too_long_rejected() {
    ...
    let res = client.try_set_proposal_metadata(&signer1, &proposal_id, &key, &long);
    assert_eq!(res.err(), Some(Ok(VaultError::MetadataValueInvalid)));
}

#[test]
fn test_metadata_entry_limit_enforced() {
    let env = Env::default();
    env.mock_all_auths();
    let (client, _admin, signer1, user, token) = make_client(&env);
...
fn test_metadata_update_existing_key_at_capacity() {
    ...
    // Updating an existing key at capacity must succeed
    let updated = String::from_str(&env, "updated");
    client.set_proposal_metadata(&signer1, &proposal_id, &Symbol::new(&env, "k1"), &updated);
